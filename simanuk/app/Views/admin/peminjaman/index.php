...
<div class="container px-6 py-8 mx-auto">
   <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-semibold text-gray-700">Daftar Peminjaman Masuk</h2>
   </div>
         ...
         <table class="w-full whitespace-no-wrap">
            ...
            <tbody class="bg-white divide-y">
               <?php if (empty($peminjaman)) : ?>
                  <tr>
                     <td colspan="5" class="px-4 py-3 text-center text-gray-500">Tidak ada data peminjaman.</td>
                  </tr>
               <?php else : ?>
                  <?php foreach ($peminjaman as $row) : ?>
                     <tr class="text-gray-700 hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm">
                           <?= esc($row['kegiatan']) ?>
                           <br>
                           <span class="text-xs text-gray-500">Ket: <?= esc(substr($row['keterangan'] ?? '-', 0, 30)) ?>...</span>
                        </td>
                        <td class="px-4 py-3 text-sm">
                           ...
                           <div class="text-xs text-gray-500 mt-1"><?= $row['durasi'] ?> Hari</div>
                        </td>
                        <td class="px-4 py-3 text-xs">
                           <?php
                           $statusColor = 'bg-gray-100 text-gray-700';
                           if ($row['status_peminjaman_global'] == 'Diajukan') $statusColor = 'bg-yellow-100 text-yellow-700';
                           if ($row['status_peminjaman_global'] == 'Disetujui') $statusColor = 'bg-blue-100 text-blue-700';
                           if ($row['status_peminjaman_global'] == 'Dipinjam') $statusColor = 'bg-indigo-100 text-indigo-700';
                           if ($row['status_peminjaman_global'] == 'Selesai') $statusColor = 'bg-green-100 text-green-700';
                           if ($row['status_peminjaman_global'] == 'Ditolak') $statusColor = 'bg-red-100 text-red-700';
                           ?>
